    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 铁通合同列表
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
...
<!-- 				<el-table-column prop="applyType" width="80" label="结算方式">
				     <template scope="scope">                    
						<p v-if="scope.row.applyType=='1'">按月</p>
						<p v-if="scope.row.applyType=='2'">按季度</p>  
						<p v-if="scope.row.applyType=='3'">按年</p>               			 
					</template>
				</el-table-column> -->
				<!-- <el-table-column prop="signingTime" width="105" label="签约时间" ></el-table-column> -->
        ...
        <!-- 新增编辑弹出框 -->
        <el-dialog v-dialogDrag :title="addOrUpTitle" :visible.sync="editVisible" width="37%" :close-on-click-modal="false" @closed="resetModalForm">
            <el-form ref="form" :model="form" :rules="rules" label-width="100px"  style="padding-right:60px;">
				...
				<el-form-item label="附件"  :required="true">
				    <yc-file-upload :upload-type="1" :file-list="fileList" @upload-file-change="uploadFileChange"></yc-file-upload>
				</el-form-item>
            </el-form>
		...
		<el-dialog class="dialogInfo" v-dialogDrag title="详情" :visible.sync="contractInfoVisiable" width="43%" :close-on-click-modal="false" @closed="resetFilePreview">
			...
			<el-row :gutter="20">
				<el-col :span="24"><div class="item-line one-line"><label>电改单价(元)：</label>{{currentRow.price}}</div></el-col>
			</el-row>
			<el-row :gutter="20">
...
<script>
import * as contractApi from '../../../api/contract.js';
        ...
        return {
			...
			methods: {
				...
				getData() {
					...
					contractApi.listContractInfo({id, name, provinceCode, cityCode, countyCode, page, size}).then(res => {
						this.loading = false;
				...
				handleDelete(id) {
					this.$confirm("您是否确认要删除该条数据？", "提示", {
					    ...
					    contractApi.delContract({id}).then(res => {
					        this.getData();
					    })
					}).catch(error => {
					    console.log(error)
				...
				saveEdit(formName) {
					this.$refs[formName].validate(valid => {
					    if (valid) {
					        ...
					        const {files,id, name, electricityUser, powerSupplier, electricityAddress, provinceCode, cityCode, countyCode,
							startTime,endTime,price,applyType,signingTime} = this.form;
					        contractApi.addOrModifyContractInfo({id,files, name, electricityUser, powerSupplier, electricityAddress,
							provinceCode, cityCode, countyCode, startTime,endTime,price,applyType,signingTime}).then(res => {
				...
				// 异步获取详情接口
				getInfoByApi (id) {
				    return contractApi.getContractInfo({id}).then(res => {
				        const { data } = res
				        return data
				    })
				},
