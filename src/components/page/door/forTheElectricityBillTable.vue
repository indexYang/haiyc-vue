    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 对下用电账单列表
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
...
<script>
import * as forTheElectricityBill from '../../../api/forTheElectricityBill.js';
export default {
    name: 'basetable',
    data() {
    ...
    methods: {
        ...
        getData() {
			this.loading = true;
            forTheElectricityBill.listForTheElectricityBill(this.query).then(res => {
				this.loading = false;
        ...
        handleDelete(id) {
            ...
            this.$confirm('确定要删除吗？', '提示', {
			   ...
			   forTheElectricityBill.delStationForTheElectricityBill({id}).then(res => {
				   this.getData();
			   })
			})
			.catch(() => {});
        ...
        async handleEdit(row) {
			...
			if(!this.form.electricityAmount){
				this.form.electricityAmount = undefined;
			}
			if(!this.form.price){
				this.form.price = undefined;
			}
			if(!this.form.deductionPrice){
				this.form.deductionPrice = undefined;
			}
        ...
        saveEdit(formName) {
			this.$refs[formName].validate(valid => {
			    if (valid) {
			        ...
			        forTheElectricityBill.addOrModifyStationForTheElectricityBill({id, userNo, month,electricityAmount,price,
		...
		// 异步获取详情接口
		getInfoByApi (id) {
		    return forTheElectricityBill.getStationForTheElectricityBill({id}).then(res => {
		        const { data } = res
		        return data
		    })
		},
		...
		// 下载模版
		downloadTemplate () {
		     let fileUrl = process.env.VUE_APP_API_URL + '/template/forTheElectricityBill_template.xlsx';  
		     location.href = fileUrl
		},
		//上传地址
		uploadUrl () {
			return process.env.VUE_APP_API_URL + '/forTheElectricityBill/importStationForTheElectricityBill.do';     
		},
		// 删除所选文件
