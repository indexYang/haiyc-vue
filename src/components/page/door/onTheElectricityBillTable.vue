    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 对上用电账单列表
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
...
<script>
import * as onTheElectricityBill from '../../../api/onTheElectricityBill.js';
export default {
    name: 'basetable',
    data() {
    ...
    methods: {
        ...
        getData() {
			this.loading = true;
            onTheElectricityBill.listOnTheElectricityBill(this.query).then(res => {
				this.loading = false;
        ...
        handleDelete(id) {
            ...
            this.$confirm('确定要删除吗？', '提示', {
			   ...
			   onTheElectricityBill.delStationOnTheElectricityBill({id}).then(res => {
				   this.getData();
			   })
			})
			.catch(() => {});
        ...
        saveEdit(formName) {
			this.$refs[formName].validate(valid => {
			    if (valid) {
			        ...
			        onTheElectricityBill.addOrModifyStationOnTheElectricityBill({id, userNo, month,electricityAmount,price,
		...
		// 异步获取详情接口
		getInfoByApi (id) {
		    return onTheElectricityBill.getStationOnTheElectricityBill({id}).then(res => {
		        const { data } = res
		        return data
		    })
		},
		...
		// 下载模版
		downloadTemplate () {
		     let fileUrl = process.env.VUE_APP_API_URL + '/template/onTheElectricityBill_template.xlsx';  
		     location.href = fileUrl
		},
		//上传地址
		uploadUrl () {
			return process.env.VUE_APP_API_URL + '/onTheElectricityBill/importStationOnTheElectricityBill.do';     
		},
		// 删除所选文件
